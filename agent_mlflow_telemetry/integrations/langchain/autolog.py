...
from ...runtime.bootstrap import TelemetryClient, initialize_telemetry
from ...runtime.config import TelemetryConfig
from .tracer import LangChainTelemetryTracer
...
class _AutologState:
    ...
    runtime: TelemetryClient | None = None
...
def autolog(
    *,
    ...
    `LangChainTelemetryTracer` automatically, so `.invoke()` works without passing
...
def enable_autolog(runtime: TelemetryClient, *, run_tracer_inline: bool = True) -> None:
    """Enable global injection of `LangChainTelemetryTracer` using the provided runtime."""
...
def _install_patch_if_needed() -> None:
    ...
    def _patched_callback_manager_init(self: Any, *args: Any, **kwargs: Any) -> None:
        ...
        for handler in handlers:
            if isinstance(handler, LangChainTelemetryTracer):
                return
    ...
    def _patched_callback_manager_merge(self: Any, *args: Any, **kwargs: Any) -> Any:
        ...
        tracer: LangChainTelemetryTracer | None = None
        duplicates: list[LangChainTelemetryTracer] = []
        for callback in inherited:
            if not isinstance(callback, LangChainTelemetryTracer):
                continue
            if tracer is None:
                tracer = callback
            else:
